            ...
            <tbody>
                <?php foreach ( $attributes as $attribute_name => $options ) : ?>
                    <tr>
                        ...
                        <td class="value">
                            <?php
                                // Obtenemos el valor seleccionado previamente si lo hay
                            ...
                            <!-- 1. Contenedor de botones personalizados (Swatches) -->
                            <div class="custom-swatches" data-attribute_name="attribute_<?php echo esc_attr( $clean_attribute_name ); ?>">
                                <?php
                                if ( ! empty( $options ) ) {
                                    foreach ( $options as $option ) {
                                        $selected_class = ( sanitize_title( $selected_value ) === sanitize_title( $option ) ) ? 'selected' : '';
                                        
                                        // Obtener nombre legible si es taxonomía global
                                        ...
                                        $option_name = apply_filters( 'woocommerce_variation_option_name', $display_name, $attribute_name, $option, $product );
                                        ?>
                                        <button type="button" class="custom-swatch-btn <?php echo esc_attr( $selected_class ); ?>" data-value="<?php echo esc_attr( $option ); ?>">
                            ...
                            <!-- 2. Select nativo oculto para sincronizar con WooCommerce y el JS -->
                            <div class="hidden-select-wrapper" style="display: none !important;">
                                <?php
